    ...
    /* [in] */ IView* view)
{
    ...
    if (id == R::id::TelephonyButton) {
        Logger::D("CActivityOne", "telephony test button clicked!");
        mHost->OnTestTelephony();
    }
...
//    CreateNavigationBar();
    ...
    mTelephonyTestButton = FindViewById(R::id::TelephonyButton);
    assert(mTelephonyTestButton != NULL);
    mTelephonyTestButton->SetOnClickListener(clickListener);
    return NOERROR;
}
...
ECode CActivityOne::OnTestTelephony()
{
    ...
    AutoPtr<IUri> uri;
    uriHelper->FromParts(IPhoneAccount::SCHEME_TEL, callNumber, String(NULL), (IUri**)&uri);
